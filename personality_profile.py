...
class Trait(TypedDict):
  overall: float
  sub: Union[SubTrait, ConscientiousnessSubTrait, ExtraversionSubTrait, NeuroticismSubTrait, OpennessSubTrait]
...
class PersonalityProfile:
  def __init__(self, profile: PersonalityProfileTemplate = None):
      ...
      self.profile: PersonalityProfileTemplate = profile if profile else {
          "agreeableness": {
              "overall": 0.0,
              "sub": {
                  "altruism": 0.0,
  ...
  def validate_and_update_profile(self, new_profile: PersonalityProfileTemplate) -> None:
  ...
  def validate_value(self, value: Union[int, float]) -> float:
      ...
      if isinstance(value, (int, float)) and 0 <= value <= 1:
          ...
          raise ValueError("Value must be a float between 0 and 1, or a positive integer.")
